...
```{r get_codebook}
codebook <- read.csv("/Users/michaelbarkasi/projects_Oviedo_lab/R_packages/FISHmQC/inst/extdata/codebook_obs.csv", row.names = 1)
```
...
```{r align_names}
summary_stats_data <- read.csv("/Users/michaelbarkasi/projects_Oviedo_lab/R_packages/FISHmQC/inst/extdata/summary_stats_all_mice.csv", row.names = 1)
...
plot(m1_summary_stats$rates)
```

Now try new function. 

```{r new_initial_estimate}
bc_counts <- m1_summary_stats$counts
names(bc_counts) <- rownames(m1_summary_stats)
cb <- as.matrix(codebook)
test_results <- mQC(bc_counts, cb, max_correctable_Hamming_distance)
```

```{r plot_results}
plot_df <- data.frame(observed = test_results$counts_provided, predicted = test_results$counts_expected_initial, bc = seq_along(bc_counts))
